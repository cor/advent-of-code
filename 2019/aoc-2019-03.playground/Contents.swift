...
let wirePointDelta: [Character: WirePoint] = [
    "R": WirePoint(x: +1, y: 0),
    "L": WirePoint(x: -1, y: 0),
    "U": WirePoint(x: 0,  y: +1),
    "D": WirePoint(x: 0,  y: -1)
...
typealias Wire = Dictionary<WirePoint, Int>
...
func generateWire(definition: WireDefinition) -> Wire {
    ...
    for wireComponent in definition {
        ...
        for _ in 0..<abs(length) {
            currentPosition += wirePointDelta[direction!]!
            wire[currentPosition] = current + 1
            current += 1;
        }
    }
...
func findIntersections(wires: [Wire]) -> Set<WirePoint> {
    return Set(wires[0].keys).intersection(Set(wires[1].keys))
}
...
// Example input
part1Solution(exampleInput1)
part1Solution(exampleInput2)

print(part1Solution(challangeInput))
...
func part2Solution(_ input: [String]) -> Int {
    ...
    return findIntersections(wires: wires)
        .map{(wires[0][$0])! + (wires[1][$0])!}
        .min()!
}

// Example input
part2Solution(exampleInput1)
part2Solution(exampleInput2)

print(part2Solution(challangeInput))
