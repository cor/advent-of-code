typealias WirePoint = (x: Int, y: Int)
typealias Wire = [WirePoint]
...
func generateWire(definition: WireDefinition) -> Wire {
    ...
    var wire: Wire = Array(repeating: (0, 0), count: length)
    var currentPosition: WirePoint = (0, 0)
    ...
    for wireComponent in definition {
        ...
        for _ in 0..<abs(length) {
            currentPosition.x += positionDelta[direction!]!.x
            currentPosition.y += positionDelta[direction!]!.y
            wire[count] = currentPosition
...
func findIntersections(wires: [Wire]) -> [WirePoint]
{
    var intersections: [WirePoint] = []
    let wire0 = wires.first!
    
    for point in wire0 {
        var isIntersection = true
        
        for wire in wires[1...] {
            if wire.filter({ $0 == point }).isEmpty {
